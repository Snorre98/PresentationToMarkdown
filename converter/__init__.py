...
# directory (or ``None`` to fall back to ``<source-folder>/markdown``).
# Internal to the library; ``output_dir`` accepts ``str | Path | None`` plus
# this callable (ADR-0028).
OutputDirResolver = Callable[[Path], "Path | None"]
...
def convert_file(
    path: str | Path,
    output_dir: str | Path | OutputDirResolver | None = None,
    ...
    ``output_dir`` defaults to ``<source-folder>/markdown`` when omitted. It may
    also be a per-file resolver ``Callable[[Path], Path | None]`` returning the
    output directory for ``path`` (or ``None`` to use the default) — see
    ADR-0028.
...
def convert_files(
    ...
    omitted; it may also be a per-file resolver ``Callable[[Path],
    Path | None]`` (ADR-0028).
